...
filter3_type = st.radio(
    "Select transform type",
    ("noop", "cartoon", "edges", "rotate"),
    key="mix-filter3-type",
)
callback = make_video_frame_callback(cast(VideoFilterType, filter3_type))
input3_video_process_track = None
if input3_ctx.output_video_track:
    input3_video_process_track = create_process_track(
        input_track=input3_ctx.output_video_track, frame_callback=callback
    )

st.header("Input 4")
input4_ctx = webrtc_streamer(
    key="input4_ctx",
    mode=WebRtcMode.SENDRECV,
    rtc_configuration=COMMON_RTC_CONFIG,
    media_stream_constraints={"video": True, "audio": False},
)
filter4_type = st.radio(
    "Select transform type",
    ("noop", "cartoon", "edges", "rotate"),
    key="mix-filter4-type",
)
callback = make_video_frame_callback(cast(VideoFilterType, filter4_type))
input4_video_process_track = None
if input4_ctx.output_video_track:
    input4_video_process_track = create_process_track(
        input_track=input4_ctx.output_video_track, frame_callback=callback
    )
...
if mix_ctx.source_video_track and input3_video_process_track:
    mix_ctx.source_video_track.add_input_track(input3_video_process_track)
if mix_ctx.source_video_track and input4_video_process_track:
    mix_ctx.source_video_track.add_input_track(input4_video_process_track)